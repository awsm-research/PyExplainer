...
import string
# import ipywidgets as widgets # please fix this issue...
from IPython.core.display import display, HTML
from IPython.display import clear_output

# not import from library but from .py file
from .logistic_regression import * 
from .boolean_rule_cg import *
# from .TED_Cartesian import *
...
class PyExplainer():
    ...
    categorical_features: a list of column names of which values are categorical
...
#         self.hbox_items = []
        ...
        self.scaler.fit(self.X_train.loc[:, self.indep])
        
        # for LLR only
        self.feature_binarizer = FeatureBinarizer()
        self.feature_binarizer.fit(X_train)
        
        # deal with categorical feature here...
        self.feature_values = {}
        self.feature_frequencies = {}
        
        self.col_names = list(X_train.columns)
        ...
        for feature in self.categorical_features:
            ...
            feature_index = self.col_names.index(feature)
            self.scaler.mean_[feature_index] = 0
            self.scaler.scale_[feature_index] = 1
#         self.feature_binarizer = fb
    ...
    def generate_instance_crossover_interpolation(self, X_test, y_test, debug=False):
        ...
        #        top_fourty_df.reset_index(inplace=True)
            ...
            # Generating 500 instances using interpolation technique
            ...
            # Generating 500 instances using cross-over technique
            ...
            new_df_case = new_df_case.drop('defect', axis=1) # label should not be included in this df -_-
    ...
    def generate_instance_random_pertubation(self, X_explain, y_explain, debug=False):
...
#             print('cat col:',column)
            values = self.feature_values[column]
            freqs = self.feature_frequencies[column]
            inverse_column = random_state.choice(values, size=num_samples,
                                                 replace=True, p=freqs)
            feature_index = self.col_names.index(column)
#             print('first row')
#             print(first_row)
#             print(first_row.shape)
            # first_row has shape (1,n_col) so must access first_row[0] first then followed by desiered index
            binary_column = (inverse_column == first_row[0][feature_index]).astype(int)
            binary_column[0] = 1
            inverse_column[0] = data[0, feature_index]
            data[:, feature_index] = binary_column
            inverse[:, feature_index] = inverse_column
    ...
    '''
        explainer: choose one of the following: rulefit, LRR, BRCG, TED 
                        LRR stands for Logistic Rule Regression
                        BRCG stands for Boolean Rule Column Generation
            LRR, BRCG, TED is from AIX360
        categorical_features: a list of column names of which values are categorical (or binary)
    '''
    
    def explain(self, X_explain, y_explain, top_k=3, max_rules=10, max_iter=10, cv=5, search_function='crossoverinterpolation', explainer='LRR', debug=False):

        search_function = search_function.lower()
        if search_function not in ['crossoverinterpolation', 'randompertubation']:
            print('wrong search function!! the search function must be "crossoverinterpolation" or "randompertubation"')
            return
        if explainer not in ['LRR', 'rulefit', 'BRCG']:
            print('wrong search function!! the search function must be "LRR", "BRCG" or "rulefit"')
            return
        ...
        if search_function == 'crossoverinterpolation':
            ...
            # This random pertubation approach to generate instances is used by LIME to gerate synthetic instances
            synthetic_object = self.generate_instance_random_pertubation( X_explain,
                                                                            y_explain,
                                                                            debug=debug)
        else:
            # TODO
        ...
        if explainer=='rulefit':
#             print('building rulefit model')
            local_model = RuleFit(rfmode='classify',
            ...
            local_model.fit(synthetic_instances.loc[:, self.indep].values, synthetic_predictions, feature_names=self.indep)
        ...
        elif explainer=='LRR' or explainer=='BRCG':
#             col_names = synthetic_instances.columns
#             min_max_scaler = MinMaxScaler()
#             scaled_synthetic_instances = min_max_scaler.fit_transform(synthetic_instances)
#             synthetic_instances = pd.DataFrame(scaled_synthetic_instances,columns=col_names)
            fb_synthetic_instances = self.feature_binarizer.transform(synthetic_instances)
            fb_X_explain =  self.feature_binarizer.transform(X_explain)
            
            if  explainer=='LRR':
                # if useOrd=True then there is error T-T
                
                local_model = LogisticRuleRegression(maxSolverIter=200)
                
            elif explainer=='BRCG':
                local_model = BooleanRuleCG(lambda0=0.01,iterMax=100, B=5, CNF=False, silent=True)
                
            local_model.fit(fb_synthetic_instances, synthetic_predictions)
            pyExp_rule_obj = {'synthetic_data': synthetic_instances,
                              'synthetic_data_fb':fb_synthetic_instances,
                              'synthetic_predictions': synthetic_predictions,
                              'X_explain': X_explain,
                              'y_explain': y_explain,
                              'X_explain_fb': fb_X_explain,
                              'indep': self.indep,
                              'dep': self.dep,
                              'local_model': local_model
                             }
...
#         elif explainer=='TED':
#             local_model = TED_CartesianExplainer(self.blackbox_model)
