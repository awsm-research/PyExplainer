    ...
    def fit(self, X, y=None, feature_names=None):
        ...
        if self.rfmode=='regress':
            ...
            self.lscv=LogisticRegressionCV(
                Cs=Cs, cv=self.cv, penalty='l2', max_iter=self.max_iter, # change penalty from l1 to l2
                tol=self.tol, n_jobs=self.n_jobs,
                random_state=self.random_state, solver='lbfgs') # change solver from liblinear to lbfgs
            self.lscv.fit(X_concat, y)
